...
export function validateEnv(): Record<keyof EnvVars, string> {
  ...
  for (const [key, config] of Object.entries<EnvVarConfig>(envVars)) {
    ...
    // For Cloud Functions, some variables are optional (like PORT, HOST)
    const isOptionalInCloudFunctions = isCloudFunctions && (key === 'PORT' || key === 'HOST');

    // Check if required
    if (config.required && !value && !isOptionalInCloudFunctions) {
    ...
    // Skip if optional in Cloud Functions and no value
    if (isOptionalInCloudFunctions && !value) {
      continue;
    }
  ...
  log.info('Environment validation successful', { 
    isCloudFunctions,
    validatedKeys: Object.keys(validatedEnv)
  });
