...
import { User, Mail, Lock, ArrowLeft, Sparkles, Check, X } from 'lucide-react';
    ...
    <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-indigo-900 via-blue-900 to-purple-800 px-4 py-8 font-poppins">
      ...
      >
          ...
          <div>
            ...
            {password && (
              <div className="mt-3 space-y-1 text-sm">
                <div className={`flex items-center gap-2 ${passwordRules.minLength ? 'text-green-400' : 'text-red-400'}`}>
                  {passwordRules.minLength ? <Check className="w-4 h-4" /> : <X className="w-4 h-4" />}
                  <span>At least 10 characters</span>
                </div>
                <div className={`flex items-center gap-2 ${passwordRules.hasUppercase ? 'text-green-400' : 'text-red-400'}`}>
                  {passwordRules.hasUppercase ? <Check className="w-4 h-4" /> : <X className="w-4 h-4" />}
                  <span>At least 1 uppercase letter</span>
                </div>
                <div className={`flex items-center gap-2 ${passwordRules.hasLowercase ? 'text-green-400' : 'text-red-400'}`}>
                  {passwordRules.hasLowercase ? <Check className="w-4 h-4" /> : <X className="w-4 h-4" />}
                  <span>At least 1 lowercase letter</span>
                </div>
                <div className={`flex items-center gap-2 ${passwordRules.hasDigit ? 'text-green-400' : 'text-red-400'}`}>
                  {passwordRules.hasDigit ? <Check className="w-4 h-4" /> : <X className="w-4 h-4" />}
                  <span>At least 1 digit</span>
                </div>
                <div className={`flex items-center gap-2 ${passwordRules.hasSpecialChar ? 'text-green-400' : 'text-red-400'}`}>
                  {passwordRules.hasSpecialChar ? <Check className="w-4 h-4" /> : <X className="w-4 h-4" />}
        ...
        <button
          ...
          className="w-full bg-white text-gray-700 font-semibold py-3 rounded-2xl hover:shadow-lg hover:scale-105 transition-all duration-200 focus:outline-none focus:ring-2 focus:ring-white focus:ring-offset-2 focus:ring-offset-transparent disabled:opacity-50 disabled:cursor-not-allowed shadow-md flex items-center justify-center gap-3"
        >
