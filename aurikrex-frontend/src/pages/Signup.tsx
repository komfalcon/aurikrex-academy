...
import { User, Mail, Phone, Lock, ArrowLeft, Sparkles, CheckCircle, XCircle, Check } from 'lucide-react';
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
          ...
          <div>
            ...
            {password && (
              <motion.div
                ...
                className="mt-3 p-3 bg-white/10 rounded-lg space-y-2"
              >
                <p className="text-xs text-white/70 font-medium mb-2">Password must contain:</p>
                <div className="grid grid-cols-1 gap-1.5 text-xs">
                  <div className={`flex items-center gap-2 transition-colors ${passwordValidation.minLength ? 'text-green-400' : 'text-red-400'}`}>
                    {passwordValidation.minLength ? <Check className="w-4 h-4" /> : <XCircle className="w-4 h-4" />}
                    <span>At least 8 characters</span>
                  </div>
                  <div className={`flex items-center gap-2 transition-colors ${passwordValidation.hasUppercase ? 'text-green-400' : 'text-red-400'}`}>
                    {passwordValidation.hasUppercase ? <Check className="w-4 h-4" /> : <XCircle className="w-4 h-4" />}
                    <span>One uppercase letter</span>
                  </div>
                  <div className={`flex items-center gap-2 transition-colors ${passwordValidation.hasLowercase ? 'text-green-400' : 'text-red-400'}`}>
                    {passwordValidation.hasLowercase ? <Check className="w-4 h-4" /> : <XCircle className="w-4 h-4" />}
                    <span>One lowercase letter</span>
                  </div>
                  <div className={`flex items-center gap-2 transition-colors ${passwordValidation.hasNumber ? 'text-green-400' : 'text-red-400'}`}>
                    {passwordValidation.hasNumber ? <Check className="w-4 h-4" /> : <XCircle className="w-4 h-4" />}
          ...
          <div className="relative my-6">
            ...
            <div className="relative flex justify-center text-sm">
              <span className="px-4 bg-gradient-to-br from-indigo-900 via-blue-900 to-purple-800 text-white/70">
                Or continue with
              </span>
            </div>
          </div>
          ...
          <button
            ...
            disabled={isLoading}
            className="w-full bg-white text-gray-800 font-semibold py-3 rounded-2xl hover:shadow-lg hover:scale-105 transition-all duration-200 focus:outline-none focus:ring-2 focus:ring-blue-400 focus:ring-offset-2 focus:ring-offset-transparent disabled:opacity-50 disabled:cursor-not-allowed shadow-md flex items-center justify-center gap-3"
          >
