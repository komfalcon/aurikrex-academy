/**
 * AnimatedBackground - Dynamic animated background with educational/science-themed objects
 * Features floating atoms, molecules, test tubes, books, mathematical symbols, planets, etc.
 * Animations are smooth, subtle, and do not distract from the main content
 * Adapts to both dark and light mode with appropriate opacity and colors
 */
import { useMemo } from 'react';
import { useTheme } from '../hooks/useTheme';

/**
 * Educational/science-themed icons as SVG components
 * Each icon accepts a key and className for React rendering and styling
 * Icons are designed to be simple, recognizable, and visually balanced
 */
const educationalIcons = [
  // Atom symbol - represents science and physics
  (key: string, className: string) => (
    <svg key={key} className={className} viewBox="0 0 24 24" fill="currentColor">
      <circle cx="12" cy="12" r="2" />
      <ellipse cx="12" cy="12" rx="10" ry="4" fill="none" stroke="currentColor" strokeWidth="1" />
      <ellipse cx="12" cy="12" rx="10" ry="4" fill="none" stroke="currentColor" strokeWidth="1" transform="rotate(60 12 12)" />
      <ellipse cx="12" cy="12" rx="10" ry="4" fill="none" stroke="currentColor" strokeWidth="1" transform="rotate(120 12 12)" />
    </svg>
  ),
  // Book icon - represents learning and reading
  (key: string, className: string) => (
    <svg key={key} className={className} viewBox="0 0 24 24" fill="currentColor">
      <path d="M4 19.5v-15A2.5 2.5 0 0 1 6.5 2H20v20H6.5a2.5 2.5 0 0 1 0-5H20" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"/>
    </svg>
  ),
  // Test tube / Flask - represents chemistry and experimentation
  (key: string, className: string) => (
    <svg key={key} className={className} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5">
      <path d="M9 3h6v8l5 9a1 1 0 0 1-.9 1.5H4.9a1 1 0 0 1-.9-1.5l5-9V3z" />
      <path d="M9 3h6" strokeLinecap="round" />
      <path d="M6 15h12" />
    </svg>
  ),
  // Math pi symbol - represents mathematics
  (key: string, className: string) => (
    <svg key={key} className={className} viewBox="0 0 24 24" fill="currentColor">
      <text x="4" y="18" fontSize="16" fontFamily="serif">π</text>
    </svg>
  ),
  // Planet / Globe - represents astronomy and global education
  (key: string, className: string) => (
    <svg key={key} className={className} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5">
      <circle cx="12" cy="12" r="10" />
      <ellipse cx="12" cy="12" rx="10" ry="4" />
      <path d="M2 12h20" />
    </svg>
  ),
  // Lightbulb (idea) - represents innovation and ideas
  (key: string, className: string) => (
    <svg key={key} className={className} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5">
      <path d="M9 21h6M12 3a6 6 0 0 0-3 11.18V17a1 1 0 0 0 1 1h4a1 1 0 0 0 1-1v-2.82A6 6 0 0 0 12 3z" strokeLinecap="round" strokeLinejoin="round" />
    </svg>
  ),
  // Graduation cap - represents achievement and graduation
  (key: string, className: string) => (
    <svg key={key} className={className} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5">
      <path d="M22 10l-10-6-10 6 10 6z" />
      <path d="M6 12v5c0 2 3 3 6 3s6-1 6-3v-5" />
      <path d="M22 10v6" strokeLinecap="round" />
    </svg>
  ),
  // DNA helix - represents biology and genetics
  (key: string, className: string) => (
    <svg key={key} className={className} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5">
      <path d="M2 15c6.667-6 13.333 0 20-6" strokeLinecap="round" />
      <path d="M2 9c6.667 6 13.333 0 20 6" strokeLinecap="round" />
      <path d="M5 12h2M17 12h2M11 6h2M11 18h2" strokeLinecap="round" />
    </svg>
  ),
  // Star - represents excellence and achievement
  (key: string, className: string) => (
    <svg key={key} className={className} viewBox="0 0 24 24" fill="currentColor">
      <polygon points="12,2 15.09,8.26 22,9.27 17,14.14 18.18,21.02 12,17.77 5.82,21.02 7,14.14 2,9.27 8.91,8.26" />
    </svg>
  ),
  // Calculator - represents computation and problem-solving
  (key: string, className: string) => (
    <svg key={key} className={className} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5">
      <rect x="4" y="2" width="16" height="20" rx="2" />
      <rect x="6" y="4" width="12" height="5" rx="1" />
      <circle cx="8" cy="13" r="1" fill="currentColor" />
      <circle cx="12" cy="13" r="1" fill="currentColor" />
      <circle cx="16" cy="13" r="1" fill="currentColor" />
      <circle cx="8" cy="17" r="1" fill="currentColor" />
      <circle cx="12" cy="17" r="1" fill="currentColor" />
      <circle cx="16" cy="17" r="1" fill="currentColor" />
    </svg>
  ),
  // Sigma (sum symbol) - represents statistics and advanced math
  (key: string, className: string) => (
    <svg key={key} className={className} viewBox="0 0 24 24" fill="currentColor">
      <text x="4" y="18" fontSize="18" fontFamily="serif">Σ</text>
    </svg>
  ),
  // Infinity symbol - represents endless learning possibilities
  (key: string, className: string) => (
    <svg key={key} className={className} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5">
      <path d="M12 12c-2-2.5-4-4-6-4a4 4 0 1 0 0 8c2 0 4-1.5 6-4zm0 0c2 2.5 4 4 6 4a4 4 0 1 0 0-8c-2 0-4 1.5-6 4z" />
    </svg>
  ),
  // Pencil - represents writing and creativity
  (key: string, className: string) => (
    <svg key={key} className={className} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5">
      <path d="M17 3a2.85 2.85 0 1 1 4 4L7.5 20.5 2 22l1.5-5.5L17 3z" strokeLinecap="round" strokeLinejoin="round" />
    </svg>
  ),
  // Molecule/benzene ring - represents organic chemistry
  (key: string, className: string) => (
    <svg key={key} className={className} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5">
      <polygon points="12,3 20,7.5 20,16.5 12,21 4,16.5 4,7.5" />
      <circle cx="12" cy="3" r="1.5" fill="currentColor" />
      <circle cx="20" cy="7.5" r="1.5" fill="currentColor" />
      <circle cx="20" cy="16.5" r="1.5" fill="currentColor" />
      <circle cx="12" cy="21" r="1.5" fill="currentColor" />
      <circle cx="4" cy="16.5" r="1.5" fill="currentColor" />
      <circle cx="4" cy="7.5" r="1.5" fill="currentColor" />
    </svg>
  ),
  // Microscope - represents research and discovery
  (key: string, className: string) => (
    <svg key={key} className={className} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5">
      <path d="M6 21h12M9 21v-3M15 21v-3" strokeLinecap="round" />
      <circle cx="12" cy="7" r="3" />
      <path d="M12 10v8" strokeLinecap="round" />
      <path d="M8 18h8" strokeLinecap="round" />
    </svg>
  ),
  // Compass - represents geometry and navigation
  (key: string, className: string) => (
    <svg key={key} className={className} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5">
      <circle cx="12" cy="12" r="10" />
      <polygon points="12,2 14.5,9.5 12,12 9.5,9.5" fill="currentColor" />
      <polygon points="12,22 9.5,14.5 12,12 14.5,14.5" fill="currentColor" opacity="0.5" />
    </svg>
  ),
];

/**
 * Animation types for variety in floating behavior
 * Each type creates a different movement pattern for visual interest
 */
type AnimationType = 'float' | 'drift' | 'orbit' | 'pulse';

/**
 * Props for individual floating item
 * Contains all properties needed to render and animate a single icon
 */
interface FloatingItem {
  id: number;
  iconIndex: number;
  size: number;
  x: number;
  y: number;
  duration: number;
  delay: number;
  rotation: number;
  animationType: AnimationType;
  opacity: number;
}

interface AnimatedBackgroundProps {
  className?: string;
  itemCount?: number;
}

/**
 * Helper function to generate floating items - called outside of render
 * Creates random positions, sizes, and animation properties for each icon
 * Distribution is designed to create visual balance across the viewport
 */
function generateFloatingItems(count: number): FloatingItem[] {
  const animationTypes: AnimationType[] = ['float', 'drift', 'orbit', 'pulse'];
  
  return Array.from({ length: count }, (_, i) => ({
    id: i,
    iconIndex: Math.floor(Math.random() * educationalIcons.length),
    size: Math.random() * 28 + 18, // 18-46px for better visibility
    x: Math.random() * 100, // 0-100%
    y: Math.random() * 100, // 0-100%
    duration: Math.random() * 25 + 20, // 20-45s animation duration (slower, more subtle)
    delay: Math.random() * -15, // Random start offset
    rotation: Math.random() * 360, // Initial rotation
    animationType: animationTypes[Math.floor(Math.random() * animationTypes.length)],
    opacity: Math.random() * 0.3 + 0.15, // 0.15-0.45 opacity for subtlety
  }));
}

export function AnimatedBackground({ className = '', itemCount = 20 }: AnimatedBackgroundProps) {
  const { theme } = useTheme();
  
  /**
   * Generate random floating items once using useMemo
   * Items are stable for the component's lifetime to prevent jarring re-renders
   */
  // eslint-disable-next-line react-hooks/exhaustive-deps
  const floatingItems = useMemo<FloatingItem[]>(() => generateFloatingItems(itemCount), []);

  /**
   * Get animation class based on animation type
   * Each type creates a distinct movement pattern
   */
  const getAnimationClass = (type: AnimationType): string => {
    switch (type) {
      case 'float':
        return 'animate-float-subtle';
      case 'drift':
        return 'animate-drift';
      case 'orbit':
        return 'animate-orbit';
      case 'pulse':
        return 'animate-float-pulse';
      default:
        return 'animate-float-subtle';
    }
  };

  return (
    <div 
      className={`fixed inset-0 overflow-hidden pointer-events-none z-0 ${className}`}
      aria-hidden="true"
    >
      {/* Gradient overlay for depth - more transparent to show floating icons better */}
      <div 
        className={`absolute inset-0 transition-opacity duration-500 ${
          theme === 'dark' 
            ? 'bg-gradient-to-br from-slate-900/80 via-slate-800/70 to-slate-900/80' 
            : 'bg-gradient-to-br from-gray-50/80 via-white/70 to-gray-100/80'
        }`}
      />
      
      {/* Floating educational icons with varied animations */}
      {floatingItems.map((item) => {
        const IconComponent = educationalIcons[item.iconIndex];
        const animationClass = getAnimationClass(item.animationType);
        
        // Calculate opacity class based on item.opacity value for consistent Tailwind usage
        // Map opacity (0.15-0.45) to Tailwind opacity classes for theme consistency
        const opacityClass = item.opacity < 0.2 
          ? 'opacity-15' 
          : item.opacity < 0.3 
            ? 'opacity-25' 
            : item.opacity < 0.4 
              ? 'opacity-35' 
              : 'opacity-45';
        
        return (
          <div
            key={item.id}
            className={`absolute ${animationClass} ${opacityClass}`}
            style={{
              left: `${item.x}%`,
              top: `${item.y}%`,
              width: `${item.size}px`,
              height: `${item.size}px`,
              animationDuration: `${item.duration}s`,
              animationDelay: `${item.delay}s`,
              transform: `rotate(${item.rotation}deg)`,
            }}
          >
            {IconComponent(
              `icon-${item.id}`,
              `w-full h-full transition-colors duration-500 ${
                theme === 'dark' 
                  ? 'text-primary-400' 
                  : 'text-primary-600'
              }`
            )}
          </div>
        );
      })}
      
      {/* Subtle radial gradients for visual interest and depth */}
      <div 
        className={`absolute w-[500px] h-[500px] rounded-full blur-3xl transition-opacity duration-500 animate-float-glow ${
          theme === 'dark' 
            ? 'bg-primary-600/8' 
            : 'bg-primary-400/12'
        }`}
        style={{ top: '5%', left: '10%' }}
      />
      <div 
        className={`absolute w-[400px] h-[400px] rounded-full blur-3xl transition-opacity duration-500 animate-float-glow ${
          theme === 'dark' 
            ? 'bg-secondary-600/8' 
            : 'bg-secondary-400/12'
        }`}
        style={{ bottom: '10%', right: '5%', animationDelay: '-5s' }}
      />
      <div 
        className={`absolute w-[300px] h-[300px] rounded-full blur-3xl transition-opacity duration-500 animate-float-glow ${
          theme === 'dark' 
            ? 'bg-primary-500/6' 
            : 'bg-primary-300/10'
        }`}
        style={{ top: '40%', right: '30%', animationDelay: '-10s' }}
      />
    </div>
  );
}
