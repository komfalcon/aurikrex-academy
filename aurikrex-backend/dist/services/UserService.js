...
export class UserService {
     ...
     */
    async listUsers(pageSize = 100, pageToken) {
        return withErrorHandling(async () => {
            try {
                ...
                const users = result.users.map(user => this.mapUserToAuthUser(user, userProfiles.get(user.uid)));
                return {
                    users,
     ...
     */
    mapUserToAuthUser(user, _profile) {
